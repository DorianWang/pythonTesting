...
from Cimpl import *
from hotmetal_lookup import *

def convolution_filter(img, kernels, name):
    """ (Cimpl.img, dict, str) -> Cimpl.img
    Return a new image created from the picture bound to img,
    using the specified 3-by-3 convolution kernel.
    Parameter kernels is a dictionary of the convolution kernels.
    Parameter name is the name of the kernel (a string) to applu to the image.
    """
...
edge1_kernel = ((0, 9, 0), (9, -36, 9), (0, 9, 0))
edge2_kernel = ((-9, -9, -9), (-9, 72, -9), (-9, -9, -9))
...
def build_kernel_table():
    table = {}
    table["blur"] = blur_kernel
    table["sharpen"] = sharpen_kernel
    table["emboss"] = emboss_kernel
    table["edge detect 1"] = edge1_kernel
    table["edge detect 2"] = edge2_kernel
